...
import { Search, Check, X, Package, ArrowRight, ArrowLeft, Tags, CheckCheck } from 'lucide-react';
...
export default function AssignProductsPage() {
  ...
  // Products assigned to selected category
  const assignedProducts = useMemo(() => {
    if (!selectedCategoryId) return [];
    return products
      .filter(p => p.categoryId === selectedCategoryId)
      .filter(p => !searchAssigned || p.name.toLowerCase().includes(searchAssigned.toLowerCase()));
  }, [products, selectedCategoryId, searchAssigned]);

  // Products not assigned to any category (available for assignment)
  const unassignedProducts = useMemo(() => {
    return products
      .filter(p => !p.categoryId)
      .filter(p => !searchUnassigned || p.name.toLowerCase().includes(searchUnassigned.toLowerCase()));
  }, [products, searchUnassigned]);

  // Products assigned to OTHER categories
  ...
  return (
    <div>
      <PageHeader
        title="Asignar Productos a Categorías"
        description={selectedCategory ? `Administra los productos de "${selectedCategory.name}"` : 'Selecciona una categoría'}
        actions={[{ label: 'Volver a Categorías', href: '/categories', variant: 'outline' }]}
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-3">
          <Card>
            <CardContent className="pt-4">
              <h3 className="font-semibold text-sm text-muted-foreground uppercase tracking-wider mb-3">Categorías</h3>
        ...
        <div className="lg:col-span-9 space-y-4">
                ...
                <div className="flex items-center gap-2">
                  {assignedProducts.length > 0 && (
                    ...
                    >
                      ...
                      {selectedToRemove.size === assignedProducts.length ? 'Deseleccionar' : 'Seleccionar todo'}
                    </Button>
                  )}
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  {unassignedProducts.length > 0 && (
                    ...
                    >
                      <CheckCheck className="h-3.5 w-3.5 mr-1" />
                      {selectedToAssign.size === unassignedProducts.length ? 'Deseleccionar' : 'Seleccionar todo'}
                    </Button>
                  )}
          ...
          {/* Products in other categories (can be moved) */}
          {otherCategoryProducts.length > 0 && (
            <Card>
              <CardContent className="pt-4">
                <div className="flex items-center gap-2 mb-3">
                  <h3 className="font-semibold text-muted-foreground">Productos en Otras Categorías</h3>
                  <Badge variant="outline">{otherCategoryProducts.length}</Badge>
                </div>
                <p className="text-xs text-muted-foreground mb-3">
                  Puedes mover estos productos a &ldquo;{selectedCategory?.name}&rdquo; (se quitarán de su categoría actual)
                </p>
                  ...
                  {otherCategoryProducts.map((product) => (
                    ...
                    >
                      <div className="flex items-center gap-2 truncate">
                        <span className="truncate">{product.name}</span>
                        <Badge variant="outline" className="text-[10px] flex-shrink-0">{product.categoryName}</Badge>
                      </div>
                      <ConfirmDialog
