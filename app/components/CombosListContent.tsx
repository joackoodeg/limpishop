...
export function CombosListContent({ initialCombos }: CombosListContentProps) {
  ...
  const generateCombosPDF = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(
          `Página ${i} de ${pageCount} | ${storeConfig.phone ? 'WhatsApp: ' + storeConfig.phone : storeConfig.storeName}`,
          margin,
          pageHeight - 10
        );
      }
    ...
    <div>
      <PageHeader title="Gestión de Combos">
        <Button
          ...
          disabled={combos.length === 0 || generatingPDF}
        >
          {generatingPDF ? (
            <>
                    ...
                    <div className="flex flex-wrap gap-2 items-start">
                      ...
                      >
                        <span className="inline-flex items-center gap-1">
                          {processingId === combo.id &&
                          processingAction === 'toggle-status' ? (
                            <>
                      ...
                      <ConfirmDialog
                        description="¿Eliminar este combo?"
                        onConfirm={() => deleteCombo(combo.id)}
