...
export default function EmpleadosPage() {
  ...
  useEffect(() => {
    if (!configLoading && isModuleEnabled('empleados')) {
      fetchEmployees();
    } else if (!configLoading) {
      setLoading(false);
    }
  }, [configLoading, fetchEmployees, isModuleEnabled]);
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              El módulo de Empleados no está habilitado. Activalo desde la configuración.
            </p>
            <Button asChild>
                  ...
                  {employeesList.map((emp) => (
                    <TableRow key={emp.id} className={!emp.active ? 'opacity-50' : ''}>
                      ...
                      <TableCell>
                        ...
                        >
                          {emp.active ? <UserCheck className="h-3 w-3" /> : <UserX className="h-3 w-3" />}
